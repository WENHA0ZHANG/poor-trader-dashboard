...
def compute_signals(
    ...
    *,
    ...
    # Top ≥ 31 / bottom ≤ 25 captures "true" extremes without firing on
    # every routine pullback.
    o = latest.get(IndicatorId.SP500_PE_RATIO)
    if o:
        v = float(o.value)
        top = v >= 31 or (pe_percentile is not None and pe_percentile >= 0.9)
        bottom = v <= 25 or (pe_percentile is not None and pe_percentile <= 0.1)
        ...
        out.append(
            Signal(
                ...
                detail=f"{v:.2f}x (>=31 top valuation; <=25 bottom valuation{pct_str})",
            )
        )

    # 7) Nasdaq 100 PE（x）
    # NDX bottoms typically 19-23x (Oct-2022 ~21, Dec-2018 ~19);
    # tops 32-40x. Top ≥30 / bottom ≤26.
    ...
    if o:
        ...
        out.append(
            Signal(
                ...
                detail=f"{v:.2f}x (>=30 top valuation; <=26 bottom valuation)",
            )
        )
    ...
    # 2.7% / 4.0% catches early warnings; >5.5% is "real bottom" zone.
    ...
    if o:
        ...
        out.append(
            Signal(
                ...
                detail=f"Current {val_display} (<2.7% credit-greed top; >4.0% credit-stress bottom)",
            )
        )
    ...
    # >=155: genuine tail-hedging spikes preceding vol events
    # <=135: low tail-hedging = complacency / bottom signal
    ...
    if o:
        ...
        out.append(
            Signal(
                ...
                detail=f"{v:.1f} (>=155 tail-hedging spike — top; <=135 low hedging — bottom)",
            )
        )
